...
def xai_score(x,y,a,net,device = torch.device( "cpu")):
    ...
    for i in range(x_b.shape[1]):
        ...
        # Quantus expects CPU attribution arrays for complexity.
...
def test_comp(x_batch,y_batch,explanations, bz=2, net=None, device = torch.device( "cpu")):
  if net is None:
      raise ValueError("test_comp requires a classifier model via net=...")
  com_t,faith_t=[],[]
  x=x_batch
  y=y_batch
  ...
  for i in range(1,length):
      ...
      com,faith=xai_score(xm,ym,exp_, net=net, device = device)
  ...
  if length==1:
      ...
      com,faith=xai_score(xm,ym,explanations,net=net,device = device)
      min_com=com
      max_faith=faith
      ind_com=1
      ind_faith=1
