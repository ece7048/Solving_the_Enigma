...
def sliding_window_inference(
    ...
    inputs2: Union[torch.Tensor , MetaTensor],
    ...
    **kwargs: Any,
    ...
    for slice_g in tqdm(windows_range) if progress else windows_range:
        ...
        if sw_batch_size > 1:
            ...
            win_data2 = torch.cat([inputs2[win_slice] for win_slice in unravel_slice]).to(sw_device)
        else:
            win_data = inputs[unravel_slice[0]].to(sw_device)
            win_data2 = inputs2[unravel_slice[0]].to(sw_device)
        if with_coord:
            seg_prob_out = predictor(win_data,win_data2, unravel_slice, *args, **kwargs)  # batched patch
            
        else:
            seg_prob_out = predictor(win_data,win_data2, *args, **kwargs)  # batched patch
            
        
        #print(seg_prob_out1.shape,seg_prob_out2.shape)
        ...
        if process_fn:
            ...
            w_t = importance_map_
            #print(importance_map_.shape, w_t.shape)
            #print('here')
