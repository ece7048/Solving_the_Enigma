...
from SR_XAI  import SwiftUnet3D
from SR_XAI.SwiftUnet3D import *
...
# https://huggingface.co/docs/transformers/model_doc/videomae#transformers.VideoMAEConfig
...
# the self-attention output is as follow: 
#1)1 value scale : then in self.vit dim=dim and the self.linear_head is uncommenet and ws=torch.zeros((x.shape[0],1),device=device)
#2)1D dim : then in self.vit dim=dim and the self.linear_head is commenet and ws=torch.zeros((x.shape[0],1,self.dim),device=device)
#3)2D dim: then in self.vit dim=dim*dim and the self.linear_head is commenet and ws=torch.zeros((x.shape[0],1,self.dim,self.dim),device=device)
#4)3D dim: then in self.vit dim=dim*dim and the self.linear_head is commenet and ws=torch.zeros((x.shape[0],1,self.dim,self.dim),device=device)
